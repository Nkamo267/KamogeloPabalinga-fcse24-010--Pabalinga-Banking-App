...
public class CustomerUI {
    ...
    private void loadAccounts() {
        try {
            accObs.clear();
            // Prefer explicit link: users.customer_id -> customers.id
            Integer linkedCid = customerUser.getCustomerId();
            List<Account> list = null;
            if (linkedCid != null) {
                list = accountService.getAccountsForCustomer(linkedCid);
            } else {
                // legacy behavior: try mapping user.id -> customerId
                list = accountService.getAccountsForCustomer(customerUser.getId());
                if (list == null || list.isEmpty()) {
                    // fallback: if username encodes customer id like "cust123", try that
                    String uname = customerUser.getUsername();
                    if (uname != null && uname.startsWith("cust")) {
                        try {
                            int cid = Integer.parseInt(uname.substring(4));
                            list = accountService.getAccountsForCustomer(cid);
                        } catch (NumberFormatException ignored) { }
                    }
                }
            }
            if (list != null) accObs.addAll(list);
