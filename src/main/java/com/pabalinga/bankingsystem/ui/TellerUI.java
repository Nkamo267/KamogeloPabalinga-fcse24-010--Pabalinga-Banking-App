...
public class TellerUI {
    ...
    public void start(Stage stage) {
        ...
        Button btnDeleteAccount = new Button("Delete Selected Account");
        ...
        btnDeleteAccount.setOnAction(e -> {
            Account selected = accList.getSelectionModel().getSelectedItem();
            if (selected == null) { new Alert(Alert.AlertType.WARNING, "Select an account first").showAndWait(); return; }
            Alert confirm = new Alert(Alert.AlertType.CONFIRMATION, "Delete account " + selected.getAccountNumber() + "? This cannot be undone.", ButtonType.YES, ButtonType.NO);
            confirm.showAndWait().ifPresent(resp -> {
                if (resp == ButtonType.YES) {
                    try {
                        accountService.deleteAccount(selected);
                        lblStatus.setText("Account deleted: " + selected.getAccountNumber());
                        loadAccounts(accObs);
                    } catch (Exception ex) { ex.printStackTrace(); new Alert(Alert.AlertType.ERROR, "Error: " + ex.getMessage()).showAndWait(); }
                }
            });
        ...
        HBox accActions = new HBox(8, btnDeleteAccount);
        accActions.setPadding(new Insets(8));
        VBox right = new VBox(8, new Label("Accounts"), accList, accActions, new Label(), lblStatus);
