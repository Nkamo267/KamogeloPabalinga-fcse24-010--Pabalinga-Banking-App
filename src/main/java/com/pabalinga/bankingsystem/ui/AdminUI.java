...
public class AdminUI {
    ...
    public void start(Stage stage) {
        ...
        btnCreateUser.setOnAction(e -> showCreateUserDialog(lblStatus, custList.getSelectionModel().getSelectedItem()));
    ...
    private void showCreateUserDialog(Label status, Customer selectedCustomer) {
        ...
        if (selectedCustomer != null) {
            vb.getChildren().add(new Label("Linking to customer: " + selectedCustomer.toString()));
        }
        ...
        dlg.showAndWait().ifPresent(resp -> {
            if (resp == ButtonType.OK) {
                try {
                    Integer cid = selectedCustomer == null ? null : selectedCustomer.getId();
                    if (cid != null && !"CUSTOMER".equals(roles.getValue())) {
                        // if linking a customer, role must be CUSTOMER
                        status.setText("Role must be CUSTOMER when linking to a customer");
                        return;
                    }
                    if (cid != null) authService.createUser(u.getText().trim(), p.getText(), roles.getValue(), cid);
                    else authService.createUser(u.getText().trim(), p.getText(), roles.getValue());
